...
|generator object | A generator object is an object with ```.__next__()``` defined. It is a type of iterator| Generator objects can be returned by generator functions, or by using the parenthesis-comprehension syntax, e.g. ```(x for x in l)```.| Generator objects get used up as you iterate through them.|
