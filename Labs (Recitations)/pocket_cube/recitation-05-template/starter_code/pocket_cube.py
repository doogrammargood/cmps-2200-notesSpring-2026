...
+----+----+----+----+----+----+----+----+
    ...
    def perform_move_sequence(self,move_sequence,mutate=True):
        ...
        if mutate:
            ...
            cube = PocketCube(state = self.state)
            for move in move_sequence:
    ...
    def correctly_placed_cubies(self, other=None, orientation: bool = False):
        ...
        if orientation==False:   
            for cubie in PocketCube.cubie_faces:
                face1,face2,face3 = PocketCube.cubie_faces[cubie]
    ...
    def scramble(self,length=15):
        ...
        scramble_sequence = np.random.choice(list(PocketCube.move_permutation_dict.keys()), size=length)
    ...
    def get_neighbors(self):
        ...
        return {(self.perform_move(move,mutate=False), move) for move in PocketCube.move_permutation_dict}
    def get_half_neighbors(self):
        '''Returns the set of cubes that can be reached using a quarter turn.'''
        return {(self.perform_move(move,mutate=False), move) for move in PocketCube.move_permutation_dict if move[-1] !="2"}
    ...
    def get_neighbors_avoiding(self, cubie):
        ...
        return {(self.perform_move(move, mutate=False), move ) for move in PocketCube.avoiding_moves[cubie]}
