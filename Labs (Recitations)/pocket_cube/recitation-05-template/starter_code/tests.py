...
class TestsOfStarterCode(TestCase):
    ...
    def test_A_star(self):
        '''
        Checks that the A star algorithm solves the cube.
        '''
        permutation_table, twist_table = create_pruning_table()
        P = PocketCube()
        P.scramble(length = 30)
        ...
        visited = A_star_solve(P, heuristic)[1]
        path = get_path_from_search(P, PocketCube(),visited)
        P.perform_move_sequence(path)
        assert P.is_solved()
class TestsOfStudentCode(TestCase):
    ...
    def test_general_solution(self):
        ...
        for scramble_seq in type(self).large_scrambles:
            ...
            assert not scrambled_cube == PocketCube() #check that the cube is scrambled.
            ...
            assert scrambled_cube==PocketCube() #check that the cube is solved.
